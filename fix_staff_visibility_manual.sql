...
-- Run this SQL in your Supabase SQL Editor to fix the issue where staff members can't see what they create
...
-- Also update the get_user_role function to be more permissive for staff who have created data
  ...
  SELECT CASE 
    WHEN EXISTS (
      ...
      WHERE p.user_id = get_user_role.user_id
        AND (p.approved = true OR p.role = 'admin')
    )
