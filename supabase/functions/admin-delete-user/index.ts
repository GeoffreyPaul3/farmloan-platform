...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!/^\b[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}\b$/.test(String(user_id))) {
    ...
    if (user_id === user.id) {
      return new Response(JSON.stringify({ error: "You cannot delete your own account" }), { status: 409, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    // Prevent deleting the last admin
    const { data: admins } = await authClient
      .from("profiles")
      .select("user_id")
      .eq("role", "admin");
    ...
    if (isTargetAdmin && adminCount <= 1) {
      return new Response(JSON.stringify({ error: "Cannot delete the last admin account" }), { status: 409, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (delErr) {
      return new Response(JSON.stringify({ error: delErr.message || "Delete failed" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
