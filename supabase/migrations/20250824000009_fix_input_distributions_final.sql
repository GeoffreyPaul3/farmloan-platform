-- Final fix for ambiguous column reference "quantity" in input_distributions
-- This avoids system triggers and focuses on the specific issue

-- 1. Drop any potential conflicting views
...
-- 2. Drop ALL user-created triggers for input_distributions
...
-- 6. Check if there are any functions that might be causing the issue
-- Drop any functions that might reference multiple tables with quantity columns
...
-- 7. Create a simple test function to verify the table structure
...
-- 8. Also fix any potential issues with input_stock policies that might be interfering
...
-- 9. Recreate the audit trigger loop WITHOUT input_distributions
...
-- 10. Check if there are any database objects that might be causing the issue
-- List all functions that reference input_distributions
DO $$
DECLARE
  func_record RECORD;
BEGIN
  ...
  LOOP
    RAISE NOTICE 'Found function that might be causing issues: %', func_record.proname;
  END LOOP;
END $$;
