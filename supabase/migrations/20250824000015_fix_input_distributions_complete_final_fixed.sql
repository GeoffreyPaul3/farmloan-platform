...
BEGIN
  ...
  -- Check ALL policies on input_distributions (FIXED: using polname instead of policyname)
  FOR obj_record IN 
    SELECT 'policy' as type, p.polname as name, pg_get_policydef(p.oid) as definition
    FROM pg_policy p
